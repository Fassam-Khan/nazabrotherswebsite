...
// lib/wordpress-api.js — add this to ALL fetch calls
...
export const getProductBySlug = async (slug) => {
  ...
  if (!res.ok || !contentType?.includes('application/json')) {
    console.error(`[getProductBySlug] Bad response for slug "${slug}":`, res.status)
...
export const getCategoryBySlug = async (slug) => {
  // ✅ If nested path came in as array, take only the last segment
  const finalSlug = Array.isArray(slug) ? slug[slug.length - 1] : slug

  const res = await fetch(
    `${WC}/products/categories?slug=${finalSlug}`,
    {
      headers: authHeader(), // ✅ Auth in header, NOT in URL
      next: { revalidate: 3600 },
    }
  )

  if (!res.ok) throw new Error(`Category "${finalSlug}" not found`)
  const data = await res.json()
  return data[0] ?? null
}
